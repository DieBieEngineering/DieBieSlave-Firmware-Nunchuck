...
void InitDefaultEntries(void)
{
...
#endif
    {
        if (pObjEntry->NonVolatileOffset != 0)
        {
            ...
            UINT32 size = OBJ_GetObjectLength( pObjEntry->Index, 0, pObjEntry, 1);
            /* call the microcontroller specific function to store the data in the non-volatile memory */
            EE_WriteWordsToNonVolatileMemory(pObjEntry->NonVolatileOffset,(UINT16 *) pObjEntry->pVarPtr, (UINT16) BYTE2WORD(size));
        }
#if !STATIC_OBJECT_DIC
